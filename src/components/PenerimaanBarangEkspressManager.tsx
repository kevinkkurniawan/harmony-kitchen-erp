...
export default function PenerimaanBarangEkspressManager({ isDark, userId }: PenerimaanBarangEkspressManagerProps) {
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem(`hk_erp_col_width_rcv_exp_${userKey}`);
    if (saved) {
  ...
  const updateNameColWidth = (newWidth: number) => {
    ...
    const userKey = userId ? `user_${userId}` : 'guest';
    localStorage.setItem(`hk_erp_col_width_rcv_exp_${userKey}`, clamped.toString());
  };

  const resetNameColWidth = () => {
    setNameColWidth(DEFAULT_NAME_WIDTH);
    const userKey = userId ? `user_${userId}` : 'guest';
    localStorage.removeItem(`hk_erp_col_width_rcv_exp_${userKey}`);
  };
