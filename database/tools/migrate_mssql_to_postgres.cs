                ...
                using (SqlDataReader reader = cmd.ExecuteReader()) {
                    using (StreamWriter sw = new StreamWriter("seed_postgres_inventory.sql")) {
                        ...
                        while (reader.Read()) {
                            ...
                            string barcode = reader.GetString(1).Replace("'", "''");
                            string invNo = reader.GetString(2).Replace("'", "''");
                            string invName = reader.GetString(3).Replace("'", "''");
                            int brandId = reader.GetInt32(4);
                            int catId = reader.GetInt32(5);
                            int prodId = reader.GetInt32(6);
                            int uomId = reader.GetInt32(7);
                            int minStock = Convert.ToInt32(reader.GetValue(8));
                            int maxStock = Convert.ToInt32(reader.GetValue(9));
                            string kodeHarga = reader.GetString(10).Replace("'", "''");
                            string desc = reader.GetString(11).Replace("'", "''");
                        ...
                        Console.WriteLine("Extracted " + count + " records from MSSQL M_Inventory");
                    }
                }
            }
        }
